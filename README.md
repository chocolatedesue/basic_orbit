...
python3 -m unittest discover -p 'test_*.py'    # 176 项解析校验
```
...
```bash
python3 cluster_dynamics.py                    # GCO vs PCO 对比 + 口径扫描
python3 cluster_dynamics.py period 650 1000 --mode pco --steps 24
python3 cluster_dynamics.py link 200           # 单条链路的高斯光束预算
```
...
## 一个周期内的集群时间序列（`cluster_dynamics.py`）

前面的模块给的都是标量指标；这个模块把 N 颗星放进 Hill/LVLH 旋转坐标系，
沿一个轨道周期传播，逐时刻算出星间距离、时延、光链路耦合效率。

有界（无长期漂移）HCW 解：

$$x = \rho_x\sin(nt+\alpha), \quad y = 2\rho_x\cos(nt+\alpha) + y_c, \quad z = \rho_z\sin(nt+\beta)$$

### 结论一：存在刚性构型，星间距离可以完全不变

取 $\rho_z = \sqrt{3}\rho_x$、$\beta = \alpha$、$y_c = 0$（GCO 构型），则任意两星间距

$$|\Delta|^2 = 4\left[\rho_1^2 + \rho_2^2 - 2\rho_1\rho_2\cos(\alpha_1 - \alpha_2)\right]$$

**不含 $t$** —— 整个集群像刚体一样旋转，所有链路预算无时变。
代价是集群退化成一个倾斜平面内的**圆盘**（$y_c$ 偏置或 $\beta \ne \alpha$ 都会破坏刚性），
真正的三维体积集群做不到刚性。

对比 PCO 构型（$\rho_z = 2\rho_x$），同环链路

$$|\Delta|^2 = 4\sin^2\phi\,(4 + \cos^2\psi), \qquad \phi = \tfrac{\alpha_1-\alpha_2}{2},\ \psi = nt + \tfrac{\alpha_1+\alpha_2}{2}$$

距离摆幅恒为 $\sqrt{5/4} - 1 = 11.8034\%$（仿真实测 11.8034%，与闭式解吻合）。

**所以"星间距离在 100–200 m 之间振荡"是构型选择的结果，不是物理必然。**

### 结论二：集群内不能套远场平方反比

集群尺度只有几百米，而 2.5 cm 口径在 1550 nm 下的瑞利距离是 **1267 m** ——
光束根本还没发散完。必须用高斯光束模型：

$$z_R = \frac{\pi w_0^2}{\lambda}, \quad w(z) = w_0\sqrt{1 + (z/z_R)^2}, \quad \eta = 1 - e^{-2a^2/w(z)^2}$$

同一个 PCO 构型（距离摆幅恒为 11.8%），耦合摆幅随口径剧变：

| 口径半径 | 瑞利距离 | 单链路耦合摆幅 | 最近邻(196m) | 跨端(2km) |
|---|---|---|---|---|
| 0.3 cm | 18 m | 0.954 dB（≈远场极限 0.97） | −17.7 dB | −37.8 dB |
| 1.0 cm | 203 m | 0.292 dB | −1.9 dB | −17.0 dB |
| 2.5 cm | 1267 m | **0.008 dB** | −0.7 dB | −3.6 dB |
| 5.0 cm | 5067 m | 0.001 dB | −0.6 dB | −0.8 dB |

远场平方反比会预测 0.97 dB；口径 ≥ 2.5 cm 时实测仅 **0.008 dB**，差两个数量级。

### 一个周期内的物理量（81 星 / 1 km 半径 / 650 km / GCO）
...
| 最近邻时延 | 0.654 μs | 0% |
| 跨端时延 | 6.671 μs | 0% |
| 最近邻耦合（2.5cm） | −0.66 dB | 0% |

### 注意：集合统计量会掩盖逐链路波动

PCO 构型下，对全体星求的"平均最近邻间距"只波动 **0.08%**，
而**单条链路**实际波动 **11.8%** —— 因为不同星的峰值出现在不同相位，
求平均把它抹平了。评估链路预算必须看逐链路量（`per_link` 字段），
集合统计量会给出错误的乐观结论。
...
- **相对运动是线性化 HCW 解**：不含 J2 对相对运动的摄动（会让"刚性"在
  长时间尺度上缓慢退化）、不含差分阻力、不含指向抖动与多普勒。
  刚性是一阶结论，真实构型需要数值验证与维持控制。
- **光链路只算几何耦合**：不含指向误差、大气（星间无）、探测器噪声、
  调制格式与编码增益。要真实速率需完整链路预算 + 误码率模型。
